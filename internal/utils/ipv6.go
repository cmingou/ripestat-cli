...
func SearchIpv6Info(ipv6s []netip.Addr, longestPrefix bool) {
	...
	results, err := runWithConcurrency(context.Background(), ipv6s, func(ctx context.Context, idx int, ip netip.Addr) ([][]string, error) {
		resource := ip.String()
	...
	if err != nil {
		fmt.Printf("Failed to get IPv6 info: %v\n", err)
		os.Exit(1)
	}
...
func SearchIpv6InfoGroupByPrefix(ipv6s []netip.Addr) {
	...
	results, err := runWithConcurrency(context.Background(), ipv6s, func(ctx context.Context, idx int, ip netip.Addr) (*PrefixInfo, error) {
		resource := ip.String()
...
func SearchIpv6InfoGroupByAsn(ipv6s []netip.Addr) {
	...
	results, err := runWithConcurrency(context.Background(), ipv6s, func(ctx context.Context, idx int, ip netip.Addr) (*ASNInfo, error) {
		resource := ip.String()
